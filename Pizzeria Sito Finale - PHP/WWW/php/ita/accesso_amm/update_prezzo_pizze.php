<?php
	try
	{
		session_start();
		//$_SESSION['amministratore']= true;
		//$_GET['tipo'] = "margherita";
		//$_GET['prezzo'] = "4,00";
		//echo"<br> sessione:";print_r($_SESSION);echo"<br>";
		//echo"<br> get:";print_r($_GET);//echo"<br>";

		$sql_sel1 = "
				select tipo 
				from pizze 
				where tipo = :tipo
		";

		$sql_up1 = "
				update pizze
				set prezzo = :prezzo
				where tipo = :tipo
		";
		
		if(isset($_SESSION['amministratore']) && !empty($_SESSION['amministratore']))
		{
			if(isset($_GET['tipo'],$_GET['prezzo']) && !empty($_GET['tipo']) && !empty($_GET['prezzo']))
			{
				$tipo = $_GET['tipo'];
				$prezzo = $_GET['prezzo'];
				$update_prezzo = false;

						require_once("../shared/credenziali_db.php");
						$dsn = "pgsql:host=$pdo_host port=$pdo_port dbname=$pdo_db user=$pdo_user password=$pdo_pass";
						$dbh = new PDO($dsn);
						$sth = $dbh -> prepare($sql_sel1);
						$sth -> bindParam(':tipo', $tipo, PDO::PARAM_STR);
							
						if($sth -> execute())
						{
							if($result = $sth -> fetchColumn())
							{
								if($result == $tipo)
								{
									
									try
									{	if($dbh->beginTransaction())
										{
											$sth = $dbh -> prepare($sql_up1);
											$sth -> bindParam(':prezzo', $prezzo);
											$sth -> bindParam(':tipo', $tipo);
										
											if($sth -> execute())
											{
												$dbh -> commit();
												$dbh = null;
												$stato = "successo";
											}
											else
											{
												$stato = "server_error";
												throw new Exception("errore execute sql_up1");
											}
										}
										else
										{
											$stato = "server_error";	
											throw new Exception("errore begin transaction sql_up1");
										}
									}
									catch(PDOException $e)
									{	
										if(isset($dbh))
										{
											$dbh -> rollback();
											$dbh = null;
										}
										if(!isset($stato) || empty($stato))
										{
											$stato = "server_error";
										}										
										//print("error: ".$e->getMessage());
									}
									catch(Exception $e)
									{
										if(isset($dbh))
										{
											$dbh -> rollback();
											$dbh = null;
										}
										if(!isset($stato) || empty($stato))
										{
											$stato = "server_error";
										}
										//print("error: ".$e->getMessage());
									}
								}
								else
								{
									$stato = "notipo";
									throw new Exception("valore tipo non presente in db");
									
								}
							}
							else
							{
								$stato = "notipo";
								throw new Exception("valore tipo $tipo non presente in db");
							}
						}
						else
						{
							$stato = "server_error";
							throw new Exception("errore execute sql_sel2");
						}
						//update prezzo in pizza
			}
			else
			{
				//echo"get non settati";
				//echo "errore server : get non settati";
				$stato = "noget";
			}

		}
		else
		{
			//echo "sessione amministratore non true";
			//echo "errore server: sessione amministratore non settata";
			$stato = "noamm";
		}
	}
	catch(PDOException $e)
	{
		if(isset($dbh))
		{
			$dbh = null;
		}
		if(!isset($stato) || empty($stato))
		{
			$stato = "server_error";
		}		
		//print("error: ".$e->getMessage());
	}
	catch(Exception $e)
	{
		if(isset($dbh))
		{
			$dbh = null;
		}
		if(!isset($stato) || empty($stato))
		{
			$stato = "server_error";
		}
		//print("error: ".$e->getMessage());
	}					
	$return = array(
	array("stato"=>$stato),
	);
	echo json_encode($return);
	
	
?>